# Phase 04 RAG Historical Checkpoint Archive

> Lookup-only historical archive. This file preserves the former checkpoint
> narrative for auditability; it is not present-state authority and is not
> normal required reading for new work. Historical uses of `current`, `next`,
> `NOT AUTHORIZED`, `selector`, `production`, and similar terms belong to the
> named checkpoint in which they were written. The current phase, production /
> default behavior, authorization, and immediate-next status are owned solely
> by `docs/current-phase.md`.

> The compact navigation/index is
> `docs/research/phase-04/p04-rag-historical-checkpoint-register.md`.

> The archived narrative preserves the contemporaneous
> Phase 04 status statements, artifact identities, decisions, and supersession
> boundaries that formerly accumulated in `docs/current-phase.md`. Any
> current-looking term below (including "current", "selector", "production",
> "authorized", or "immediate next action") is scoped to its named historical
> checkpoint. It is not present-day project authority. Current phase status,
> production/default behavior, and authorization are owned solely by
> `docs/current-phase.md`.

## Historical active-phase snapshot
...
## Historical P04 work-unit record
...
## Historical post-W7 direction update
...
## Historical post-adoption authority snapshot
...
At that checkpoint, the immediate next action was the provider-free local
BGE-vs-Qwen 70Q comparison. It had to hold the same accepted 535,802-RU corpus, lexical/BM25
...
## Later blind Packet-semantic review checkpoint

This is a later accepted checkpoint after the corrected fixed-control
BGE-Qwen mechanical comparison above. Two independent reviewers received only
the question and blinded Packet A/B before unblinding. The original reviewer
artifacts remain external/untracked; this repository records the accepted
summary, identity hashes, and claim boundary only.

Reviewer 1 labels were A_BETTER `2`, B_BETTER `4`,
BOTH_SUFFICIENT_EQUIVALENT `51`, BOTH_INSUFFICIENT `13`, and INDETERMINATE `0`.
Reviewer 2 labels were A_BETTER `2`, B_BETTER `2`,
BOTH_SUFFICIENT_EQUIVALENT `51`, BOTH_INSUFFICIENT `15`, and INDETERMINATE `0`.
Exact primary-label agreement was `63/70 = 90%`; no question had one reviewer
select A_BETTER and the other select B_BETTER.

Frozen blind consensus before unblinding was A_BETTER `3`, B_BETTER `4`,
BOTH_SUFFICIENT_EQUIVALENT `49`, BOTH_INSUFFICIENT `14`, and INDETERMINATE `0`.
The consensus JSONL SHA-256 is
`910e3ccfe92783ff32e46c1c67494d607a120ab15fe8ee69e92d1e4c11596e94`; the
consensus summary SHA-256 is
`d9de573c165597939d3a0a0509b38db3f050e6584993e28b24618229c4c6837d`.
The accepted unblinding map SHA-256 is
`17aa4f614f828ffcf929a48e4c4e6540362e378c8dbaf63ede2195772ce2f692`; its
mapping is complete `70/70` and bound to the corrected comparison descriptor.

After unblinding, the accepted labels were QWEN_BETTER `5`, BGE_BETTER `2`,
BOTH_SUFFICIENT_EQUIVALENT `49`, BOTH_INSUFFICIENT `14`, and INDETERMINATE `0`.
Directional Qwen questions were Q004 (medium), Q023 (medium), Q041 (high),
Q061 (high), and Q064 (high). Directional BGE questions were Q022 (high) and
Q057 (medium). Both-insufficient questions were Q003, Q005, Q008, Q011, Q015,
Q033, Q044, Q045, Q046, Q049, Q050, Q058, Q060, and Q070.

The unblinded JSONL SHA-256 is
`2ec71922951d95825f9d01b6ccb21f7641688687dd22a446bd724d03ecd4fc1c`; the
unblinded summary SHA-256 is
`f21c8f0f9e147cebadf2cfd2e8c9b1f8200aece4d747d2cd1975e4cd90190825`.

Decision boundary: this supports only a slight directional Qwen advantage in
fixed-control 70Q Evidence Packet semantic utility. Overall BGE and Qwen
Packet semantic utility remains highly similar: `49/70` are
both-sufficient/equivalent, `14/70` are both-insufficient, and only `7/70`
have a directional winner. It is not a benchmark-winner claim, does not
establish clear Qwen superiority, and does not authorize production Dense
adoption. It does not establish Answer-level correctness, completeness,
citation faithfulness, or hallucination improvement.
...
At the preserved checkpoint, the prior `docs/current-phase.md` was the source
of current stage and immediate next action. Today those facts are owned solely
by the live `docs/current-phase.md`.
